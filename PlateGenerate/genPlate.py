...
def GenCh1(f,val):
    ...
    生成英文字符
    """
    img=Image.new("RGB", (22,65),(255,255,255))
    draw = ImageDraw.Draw(img)
    r = 164
    g = 186
    b = 209
    draw.text((0, 2),val.decode('utf-8'),(255-r,255-g,255-b),font=f)
    A = np.array(img)
    return A
...
class GenPlate:
    ...
    def __init__(self,fontCh,fontEng,NoPlates):
        ...
        self.fontE =  ImageFont.truetype(fontEng,50,0);
        self.img=np.array(Image.new("RGB", (226,70),(255,255,255)))
        self.bg  = cv2.resize(cv2.imread("./images/VIC_template.jpg"),(226,70));
    ...
    def draw(self,val):
        ...
        for i in range(6):
            ...
            base = offset +i*22 + i*7 ;
            if i >2:
                base+=32
            self.img[5:70, base  : base+22]= GenCh1(self.fontE,val[i]);
	...
	生成车牌String,存为图片
        ...
        # plate.extend(b[1:7])
        plate = [plateList[i][-1] for i in range(len(plateList))]
	...
	outfile = open('label.txt','w')
		...
		img =  G.generate(plateStr);
                ...
                cv2.imwrite(outputPath + "/" + str(i+5000).zfill(2) + ".jpg", img);
		outfile.write(str(i+5000).zfill(2) + ".jpg"+":"+plateStr+"\n")
G = GenPlate("./font/platech.ttf",'./font/VicLicensePlate.ttf',"./NoPlates")
#G.genBatch(100,2,range(31,65),"./plate_100",(272,72))

if __name__=='__main__':
    G.genBatch(int(sys.argv[1]),2,range(31,65),sys.argv[2],(164,48))
